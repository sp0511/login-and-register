jQuery(document).ready(function ($) {
    ...
    function startTimer(seconds, element) {
        ...
        const interval = setInterval(function() {
            ...
            timerElement.text(`(${minutes}:${seconds < 10 ? '0' : ''}${seconds} صبر کنید)`);
            timeLeft--;
    ...
    $('#custom-register-form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.success) {
                    $('#register-message').html('<p class="success">' + data.data.message + '</p>');
                    $('#custom-register-form').hide();
                    $('#otp-form').show();
                    $('#otp_mobile').val(data.data.mobile);
                    $('#otp_user_id').val(data.data.user_id);
                    startTimer(data.data.wait_time, $('#resend-timer'));
                } else {
                    $('#register-message').html('<p class="error">' + data.data + '</p>');
                }
            },
            error: function() {
    ...
    $('#custom-login-form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success) {
                    $('#login-message').html('<p class="success">' + data.data + '</p>');
                    setTimeout(function() {
                        window.location.reload();
                    }, 2000);
                } else {
                    $('#login-message').html('<p class="error">' + data.data + '</p>');
                }
            },
            error: function() {
    ...
    $('#custom-forgot-password-form').on('submit', function(e) {
        ...
        $.ajax({
            type: 'POST',
            url: customLoginRegister.ajax_url,
            data: {
                action: 'custom_forgot_password',
                nonce: customLoginRegister.forgot_password_nonce,
                forgot_credential: $('#forgot_credential').val()
            },
            success: function(data) {
                if (data.success) {
                    $('#forgot-password-message').html('<p class="success">' + data.data.message + '</p>');
                    $('#custom-forgot-password-form').hide();
                    $('#otp-form-forgot').show();
                    $('#otp_mobile_forgot').val(data.data.mobile);
                    $('#otp_user_id_forgot').val(data.data.user_id);
                    startTimer(data.data.wait_time, $('#resend-timer-forgot'));
                } else {
                    $('#forgot-password-message').html('<p class="error">' + data.data + '</p>');
                }
            },
            error: function() {
    ...
    $('#custom-verify-otp-form').on('submit', function(e) {
        e.preventDefault();
        $.ajax({
            type: 'POST',
            url: customLoginRegister.ajax_url,
            data: {
                ...
                nonce: customLoginRegister.verify_otp_nonce,
            ...
            success: function(data) {
                if (data.success) {
                    $('#otp-message').html('<p class="success">' + data.data + '</p>');
                    setTimeout(function() {
                        window.location.href = '<?php echo home_url(); ?>';
                    }, 2000);
                } else {
                    $('#otp-message').html('<p class="error">' + data.data + '</p>');
                }
            },
            error: function() {
                $('#otp-message').html('<p class="error">خطایی رخ داد. لطفاً دوباره تلاش کنید.</p>');
            }
        });
    });
    ...
    $('#custom-verify-otp-forgot-form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.success) {
                    $('#otp-message-forgot').html('<p class="success">' + data.data.message + '</p>');
                    if (data.data.show_reset_form) {
                        $('#otp-form-forgot').hide();
                        $('#reset-password-form').show();
                        $('#reset_user_id').val($('#otp_user_id_forgot').val());
                    }
                } else {
                    $('#otp-message-forgot').html('<p class="error">' + data.data + '</p>');
                }
            },
            error: function() {
    ...
    $('#custom-reset-password-form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            data: {
                ...
                nonce: customLoginRegister.reset_password_nonce,
            ...
            success: function(data) {
                if (data.success) {
                    $('#reset-password-message').html('<p class="success">' + data.data + '</p>');
                    setTimeout(function() {
                    ...
                    $('#reset-password-message').html('<p class="error">' + data.data + '</p>');
                }
            },
            error: function() {
    ...
    $('#resend-otp, #resend-otp-login, #resend-otp-forgot').on('click', function() {
        ...
        $.ajax({
            ...
            data: {
                ...
                nonce: customLoginRegister.resend_otp_nonce,

...
class Custom_Login_Register_OTP_Handler {
    ...
    public function generate_otp( $user_id, $mobile ) {
        ...
        $row = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT attempt_count, last_attempt_at FROM $table_name WHERE user_id = %d AND mobile = %s",
                $user_id,
                $mobile
            )
        );
        ...
        if ( $row ) {
            if ( $row->attempt_count >= $max_attempts ) {
                ...
                if ( ( time() - $last_attempt_time ) < $lockout_duration ) {
                    ...
                    $wpdb->delete( $table_name, [ 'user_id' => $user_id, 'mobile' => $mobile ], [ '%d', '%s' ] );
                }
            }
        }

        $otp_code = sprintf( '%04d', mt_rand( 100000, 999999 ) );
    ...
    public function verify_otp( $mobile, $otp_code ) {
        global $wpdb;
        $table_name = $wpdb->prefix . 'custom_otp_codes';

        $row = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT * FROM $table_name WHERE mobile = %s AND otp_code = %s AND expires_at > %s",
                $mobile,
                $otp_code,
                current_time( 'mysql' )
            )
